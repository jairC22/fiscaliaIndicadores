...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('extension')->default('sin registrar'); // Extensión telefónica interna
            ...
            $table->date('fecha_ingreso')->nullable(); // Fecha en la que ingresó a la fiscalía
            $table->timestamp('email_verified_at')->nullable(); //se verifica el email del susuario enviando un correo electronico
            $table->string('password');
            $table->boolean('estado')->default(true);
            $table->unsignedBigInteger('fiscalia_fk')->nullable();
            $table->unsignedBigInteger('roles_fk')->nullable();
            //$table->unsignedBigInteger('session_fk')->nullable();
            $table->rememberToken();
            $table->timestamps();
        });
